...
import { ChevronLeft, ChevronRight, Calendar, Trophy, User, DollarSign } from 'lucide-react';
...
export default function AgentRankings() {
  ...
  useEffect(() => {
    // Simulate API fetch
      ...
      // fetch('/api/rankings')
        ...
        fetchData(import.meta.env.VITE_AGENT_RANKINGS)
            .then((rankingData)=>{
                setData(rankingData)
            })
  ...
  const getCurrentMonthData = () => {
    ...
    const rankingsArray = Object.entries(data[currentMonth]).map(([rank, details]) => ({
      rank: parseInt(rank),
      agent: details.agent,
      ...
      <div className="w-full max-w-4xl">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex items-center bg-gray-100 dark:bg-gray-800 p-1 rounded">
            <span className="px-3 text-gray-700 dark:text-gray-300">SORT BY:</span>
            <button 
              className={`flex items-center px-4 py-2 rounded ${sortBy === 'rank' ? 'bg-blue-600 text-white' : 'text-gray-700 dark:text-gray-300'}`}
            ...
            <button 
              className={`flex items-center px-4 py-2 rounded ${sortBy === 'name' ? 'bg-blue-600 text-white' : 'text-gray-700 dark:text-gray-300'}`}
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg overflow-hidden shadow">
          ...
          {loading ? (
            ...
            getCurrentMonthData().map((item) => (
              <div key={item.rank} className="grid grid-cols-3 p-4 border-b border-gray-100 dark:border-gray-700 items-center">
                <div className="flex items-center">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center ${getRankColor(item.rank)}`}>
                    {item.rank}
                  </div>
                </div>
